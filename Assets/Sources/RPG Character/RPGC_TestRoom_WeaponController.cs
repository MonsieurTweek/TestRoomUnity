...
namespace RPGC_TestRoom_Anims{
	...
	public class RPGC_TestRoom_WeaponController : MonoBehaviour{
...
//			Debug.Log("Switch Weapon: " + weaponNumber);
		...
		public IEnumerator _UnSheathWeapon(int weaponNumber){
			...
			RPGC_TestRoom_Controller.SetWeaponState(weaponNumber);
			yield return null;
		}
		...
		public IEnumerator _SheathWeapon(int weaponNumber, int weaponTo){
			...
			RPGC_TestRoom_Controller.SetWeaponState(weaponTo);
			yield return null;
		}
		...
		void DoWeaponSwitch(int weaponSwitch, int weaponVisibility, int weaponNumber, int leftRight, bool sheath){
			...
			if(weaponSwitch < 1){
				if(Is2HandedWeapon(weaponNumber)){
					RPGC_TestRoom_Controller.Lock(true, true, true, 0f, 1f);
				}
			}
			...
			if(sheath){
				...
				//If using RPGC_TestRoom_IKHands, trigger IK blend.
				if(RPGC_TestRoom_Controller.RPGC_TestRoom_IKHands != null){
					StartCoroutine(RPGC_TestRoom_Controller.RPGC_TestRoom_IKHands._BlendIK(false, 0f, 0.2f, weaponVisibility));
				}
			}
			else{
				...
				//If using RPGC_TestRoom_IKHands, trigger IK blend.
				if(RPGC_TestRoom_Controller.RPGC_TestRoom_IKHands != null){
					StartCoroutine(RPGC_TestRoom_Controller.RPGC_TestRoom_IKHands._BlendIK(true, 0.5f, 1, weaponVisibility));
				}
			}
		}
		...
		public IEnumerator _HideAllWeapons(bool timed, bool resetToUnarmed){
			...
			if(resetToUnarmed){
				...
				StartCoroutine(RPGC_TestRoom_Controller.RPGC_TestRoom_WeaponController._WeaponVisibility(RPGC_TestRoom_Controller.RPGC_TestRoom_WeaponController.leftWeapon, false, true));
		...
		void SetAnimator(int weapon, int weaponSwitch, int Lweapon, int Rweapon, int weaponSide){
			...
			RPGC_TestRoom_Controller.SetWeaponState(weapon);
		}
