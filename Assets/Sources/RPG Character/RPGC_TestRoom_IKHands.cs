...
namespace RPGC_TestRoom_Anims{
	...
	public class RPGC_TestRoom_IKHands : MonoBehaviour{
		private Animator animator;
		private RPGC_TestRoom_WeaponController RPGC_TestRoom_WeaponController;
		...
		public IEnumerator _BlendIK(bool blendOn, float delay, float timeToBlend, int weapon){
			...
			RPGC_TestRoom_WeaponController.isSwitchingFinished = true;
			yield break;
		}

		void GetCurrentWeaponAttachPoint(int weapon){
			if(weapon == 1){
				blendToTransform = RPGC_TestRoom_WeaponController.twoHandSword.transform.GetChild(0).transform;
			}
		}
	}
}